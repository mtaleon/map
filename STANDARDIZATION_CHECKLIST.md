# Standardization Checklist - Map Coloring

## Phase 1: Foundation
- [x] config.json created with DEFAULT_CONFIG fallback in app.js
- [x] UUID system with game_id prefix (`octile:map:*_uuid`)
- [x] i18n audit complete (no hardcoded UI strings)
- [x] localStorage keys use `octile:map:{feature}:{name}` pattern
...
- [x] Queue key: `octile:map:api_queue`

## Phase 3: User-Facing
- [x] In-game help modal (i18n applied)
...
- [x] SessionManager with game_id prefix (`octile:map:admob_session`)
- [x] AdMobManager with game_id metrics (`octile:map:admob_metrics`)
- [x] Octile branding (footer + help cross-promo)
...
- [x] **i18n old key migration**: Map migrates old lang key → new game_id key (`octile:map:lang`)
...
- [x] **AdMob keys game-scoped**: `octile:map:admob_session`, `octile:map:admob_metrics`
